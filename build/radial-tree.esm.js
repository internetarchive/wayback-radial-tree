...
import { arc as arc$1 } from 'd3-shape';
...
const arc = arc$1().startAngle(d => d.x0).endAngle(d => d.x1).innerRadius(d => Math.sqrt(d.y0)).outerRadius(d => Math.sqrt(d.y1));
...
function createVisualization(element, vis, radius, baseURL, currentYear, data, height) {
  const partition$1 = partition().size([2 * Math.PI, radius * radius]);
...
function RadialTree(element, cdxData) {
  ...
  function renderChart(element, currentYear) {
    ...
    const vis = select('#chart').append('svg:svg').attr('width', width + legendWidth).attr('height', height).append('svg:g').attr('id', 'd3_container').attr('transform', 'translate(' + width / 2 + ',' + height / 2 + ')');
